...
export class ArtClass extends ArtObject {
    ...
    get class_loader(): HeapReference<ArtClassLoader> {
        return new HeapReference((handle) => new ArtClassLoader(handle), this.handle.add(ArtClass.HeapReferenceSize * 0))
    }

    // HeapReference<Class> component_type_;
    get component_type(): HeapReference<ArtClass> {
        return new HeapReference((handle) => new ArtClass(handle), this.handle.add(ArtClass.HeapReferenceSize * 1))
    }

    // HeapReference<DexCache> dex_cache_;
    get dex_cache(): HeapReference<DexCache> {
        return new HeapReference((handle) => new DexCache(handle), this.handle.add(ArtClass.HeapReferenceSize * 2))
    }

    // HeapReference<ClassExt> ext_data_;
    get ext_data(): HeapReference<ClassExt> {
        return new HeapReference((handle) => new ClassExt(handle), this.handle.add(ArtClass.HeapReferenceSize * 3))
    }

    // HeapReference<IfTable> iftable_;
    get iftable(): HeapReference<IfTable> {
        return new HeapReference((handle) => new IfTable(handle), this.handle.add(ArtClass.HeapReferenceSize * 4))
    }

    // HeapReference<String> name_;
    get name(): HeapReference<StdString> {
        return new HeapReference((handle) => new StdString(handle), this.handle.add(ArtClass.HeapReferenceSize * 5))
    }

    // HeapReference<Class> super_class_;
    get super_class(): HeapReference<ArtClass> {
        return new HeapReference((handle) => new ArtClass(handle), this.handle.add(ArtClass.HeapReferenceSize * 6))
    }

    // HeapReference<PointerArray> vtable_;
    get vtable(): HeapReference<NativePointer> {
        return new HeapReference((handle) => new NativePointer(handle), this.handle.add(ArtClass.HeapReferenceSize * 7))
    }
