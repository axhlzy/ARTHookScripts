 ...
 * @returns 
 */
function dump_mem(from: NativePointer, length: number, fileName?: string, path?: string, withLog: boolean = true) {
    if (length <= 0) return

    let savedPath: string = path == undefined ? getFilesDir() : path
...
declare global {
    ...
    var dumpMem: (from: NativePointer, length: number, fileName?: string, path?: string, withLog?: boolean) => void
}
